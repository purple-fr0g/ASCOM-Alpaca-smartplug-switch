...
import smartplug_driver.devname_driver as devname_driver
...
logger: Logger = None
is_connected = [False for _ in range(len(Config.devices))]
supported_devices = {
    "device-name" : devname_driver,
    "another-device" : None
}
...
def start_switch_device(logger_instance: Logger):
    ...
    # TODO: assert that all drivers exist
    for device in Config.devices:
        assert(device["driver"] in  supported_devices.keys())
...
def get_device_addr(devnum: int) -> str:
    return Config.devices[devnum]["addr"]

def get_device_name(devnum: int) -> str:
    return Config.devices[devnum]["name"]

def get_device_size(devnum: int) -> int:
    return len(Config.devices[devnum]["switch"])

def get_device_switch_description(devnum: int, switch_id: int) -> str:
    return Config.devices[devnum]["switch"][switch_id]["description"]

def get_device_switch_name(devnum: int, switch_id: int) -> str:
    return Config.devices[devnum]["switch"][switch_id]["name"]

def set_device_switch_name(devnum: int, switch_id: int, name: str) -> None:
    Config.devices[devnum]["switch"][switch_id]["name"] = name

def set_device_switch_description(devnum: int, switch_id: int, description: str) -> None:
    Config.devices[devnum]["switch"][switch_id]["description"] = description

def get_device_driver(devnum: int) -> ModuleType:
    return supported_devices[Config.devices[devnum]["driver"]]
...
class connect:
    def on_put(self, req: Request, resp: Response, devnum: int):
        global is_connected
        try:
            test = get_device_driver(devnum).get_power_status(Config.devices[devnum]["addr"])
...
class connected:
    ...
    def on_put(self, req: Request, resp: Response, devnum: int):
        ...
        try:
            if conn:
                get_device_driver(devnum).get_power_status(Config.devices[devnum]["addr"])
                is_connected[devnum] = True
            else:
